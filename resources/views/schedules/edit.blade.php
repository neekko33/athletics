...
<div class="container mx-auto max-w-2xl">
    ...
    <div class="bg-white p-6 rounded-md shadow-md">
        ...
        <form action="{{ route('competitions.schedules.update', [$competition, $schedule]) }}" 
              method="POST">
            @csrf
            @method('PUT')
                ...
                <select name="heat_id" 
                    ...
                    @foreach($availableHeats as $availableHeat)
                        <option value="{{ $availableHeat->id }}" 
                                {{ (old('heat_id', $schedule->heat_id) == $availableHeat->id) ? 'selected' : '' }}>
            ...
            <div class="my-5">
                ...
                <input type="datetime-local" 
                       name="end_at" 
                       id="end_at"
                       value="{{ old('end_at', $schedule->end_at ? $schedule->end_at->format('Y-m-d\TH:i') : '') }}"
            ...
            <div class="inline mt-6">
                <button type="submit" class="btn btn-primary mr-2">更新日程</button>
